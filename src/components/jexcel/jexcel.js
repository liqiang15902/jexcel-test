...
if (! jSuites && typeof(require) === 'function') {
    var jSuites = require('./jsuites');     // modify by hubo, 调用本地版本
    require('./jsuites.css');       // modify by hubo, 调用本地版本
}
...
;(function (global, factory) {
        ...
        */
        ...
        obj.createCellHeader = function(colNumber) {
            ...
            if (obj.options.columns[colNumber].title) {
                // Modify by 胡波title使用错了变量，在官方4.9.6版本中已经修复
                ...
                if (obj.selectedCell[1] == 0 || obj.selectedCell[3] == 0) {
                    ...
                    if (obj.loadValidation()) {
                        ...
                        if (obj.selectedCell[1] - item < 30) {
                            obj.loadUp();                            obj.updateSelectionFromCoords(obj.selectedCell[0], obj.selectedCell[1], obj.selectedCell[2], obj.selectedCell[3]);
                        }
                    }
                }
                        ...
                        onclick:function() {
                            if (obj.selectedCell) {
                                navigator.clipboard.readText().then(function(text) {
                                    if (text) {
                                        console.log(obj.selectedCell[0], obj.selectedCell[1],'2222')
            ...
            if (jexcel.current.edition) {
                if (e.which == 27) {
                    ...
                    if (jexcel.current.options.columns[jexcel.current.edition[2]].type == 'calendar' ||
                        jexcel.current.options.columns[jexcel.current.edition[2]].type == 'mytext') {   // add by hubo, 增加自定义版本
                ...
                if (e.which == 37) {
                    ...
                    if ((e.ctrlKey || e.metaKey) && ! e.shiftKey) {
                        if (e.which == 65) {
                            ...
                            // Ctrl + S
                            // jexcel.current.download();   // by hubo，暂不支持
                            e.preventDefault();
                        } else if (e.which == 89) {
                            // Ctrl + Y
                            // jexcel.current.redo();   // by hubo，暂不支持
                            e.preventDefault();
                        } else if (e.which == 90) {
                            // Ctrl + Z
                            // jexcel.current.undo();   // by hubo，暂不支持
                            e.preventDefault();
                        } else if (e.which == 67) {
                            // Ctrl + C
    ...
    jexcel.contextMenuControls = function(e) {
        ...
        if (jexcel.current) {
            /*if (jexcel.current.edition) {     // Modify by hubo, 定义的edition也允许右键
                e.preventDefault();
                ...
                if (jexcel.current) {
                    // var x = e.target.getAttribute('data-x');
                    // var y = e.target.getAttribute('data-y');
                    // Modify by hubo, 通过e.path逐个查找x和y，支持自定义jexcel的类型的时候，使用各种复杂的展示组件
                    var x = null;
                    var y = null;
                    ...
                    if (x || y) {
                        ...
                        {
                            if (x !== null && y !== null) jexcel.current.updateSelectionFromCoords(x, y, x, y); // modify by 胡波，增加条件判断，选择标题的时候，这里会出错
                        }
